...
public class SolarSystem : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        rb = new Rigidbody[celestials.Length];
       
        for (int i = 0; i < celestials.Length; i++)
        {
            rb[i] = celestials[i].GetComponent<Rigidbody>();
        }
    ...
    void Gravity()
    {
        foreach (GameObject a in celestials)
        {
            foreach (GameObject b in celestials)
            {
                if (!a.Equals(b))
                {
                    float m1 = rb[Array.IndexOf(celestials, a)].mass;
                    float m2 = b.GetComponent<Rigidbody>().mass;
                    float r = Vector3.Distance(a.transform.position, b.transform.position);

                    // a.GetComponent<Rigidbody>().AddForce((b.transform.position - a.transform.position).normalized * (G * (m1 * m2) / (r * r)));
                    rb[Array.IndexOf(celestials,a)].AddForce((b.transform.position - a.transform.position).normalized * (G * (m1 * m2) / (r * r)));
                }
            }
        }
    }
